...
(defn parse
  ...
  (doseq [line (clojure.string/split-lines data)]
    ...
    (if-let [groups (re-matches #"^(\w+)\s+(\d+)\s+(\d+)(.*)" line)]
        ((def new-item (struct knapsack-item (nth groups 1) (Integer/parseInt (nth groups 2)) (Integer/parseInt (nth groups 3)))) ;fixme
